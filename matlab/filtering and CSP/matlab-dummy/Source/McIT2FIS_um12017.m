...
elseif(nargin == 2)
    ...
    Cf = MODEL.CENTER_forward;
    Cb = MODEL.CENTER_backward;
...
if (train == true)
    ...
    Cf = zeros(sum(NHN),NIP);
    Cb = zeros(sum(NHN),NIP);
    ...
    R(1,UY(1,end)) = 1;
    Cl(sum(R),:) = UY(1,1:NIP)-0.1;
    Cr(sum(R),:) = UY(1,1:NIP)+0.1;
    %S(sum(R),1) =kappa *sqrt((1/NIP)*Cl(sum(R),:)*Cl(sum(R),:)');
    S(sum(R),1) =sqrt((1/NIP)*Cl(sum(R),:)*Cl(sum(R),:)');
    A(sum(R),sum(R)) = 1;  
...
%             % Calculate knowledge, error in sample
            [~,ll]=find(F'>0.001);
            
            if ~isempty(ll)
                SP = sum(F(ll))/length(ll);
            else
                SP = sum(F)/sum(R);
            end
%          
...
%             keyboard
%             disp('    c         ccap     Err_m        Ea      max(phi)      SP         En');
%              disp([c ccap Err_m  Ea max(phi)  SP  En ] );



            if( (c~=ccap && (Err_m>Ea || max(phi) < 1e-01) && SP<En && R(1,c)<NHN(1,c))|| R(1,c) == 0 || (c==ccap && SP<0.001 && R(1,c)<NHN(1,c)))
            %if( (c~=ccap && (Err_m>Ea || max(phi) < 1e-01) && SP<En && R(1,c)<NHN(1,c))|| R(1,c) == 0 )
                ...
                try
                       % W(1:sum(R),:) = pinv(A(1:sum(R),1:sum(R)))*B(1:sum(R),:);
                       %keyboard
                   % W(1:sum(R),:) = pinv(A(1:sum(R),1:sum(R))+ size(l_samples,2)* 0.001*eye(sum(R)))*B(1:sum(R),:);
                    W(1:sum(R),:) = pinv(A(1:sum(R),1:sum(R))+  0.01*eye(sum(R)))*B(1:sum(R),:);
                catch ME
                        %  W(1:sum(R),:) = inv(A(1:sum(R),1:sum(R)))*B(1:sum(R),:);
                   % W(1:sum(R),:) = inv(A(1:sum(R),1:sum(R))+size(l_samples,2)*0.001*eye(sum(R)))*B(1:sum(R),:);
                    W(1:sum(R),:) = inv(A(1:sum(R),1:sum(R))+0.01*eye(sum(R)))*B(1:sum(R),:);
                end
            elseif( c == ccap && Err_m>El)
               ...
               [~,l_samples] = find(flag == 1);
                ...
                %W(1:sum(R),:) = pinv(A(1:sum(R),1:sum(R)))*B(1:sum(R),:);
%                W(1:sum(R),:) = W(1:sum(R),:)+pinv(A(1:sum(R),1:sum(R)) + size(l_samples,2)*0.001*eye(sum(R))      )*phi*Err;
                 W(1:sum(R),:) = W(1:sum(R),:)+pinv(A(1:sum(R),1:sum(R)) + 0.01*eye(sum(R))      )*phi*Err;
