function [varargout] = McIT2FIS_partial(varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if (train == true)
    ...
    R(1,UY(1,end)) = 1;
    Cl(sum(R),:) = UY(1,1:NIP)-0.01;
    Cr(sum(R),:) = UY(1,1:NIP)+0.01;
    ...
    while TRA     % Continue learning rest of the samples
        ...
        for SAMP = 1:length(pre_samples)
            ...
            %Handling samples withput labels
            if (c==0)
...
%             end
            ...
            elseif( c == ccap && Err_m>El)
                ...
                %W(1:sum(R),:) = pinv(A(1:sum(R),1:sum(R)))*B(1:sum(R),:);
                W(1:sum(R),:) = W(1:sum(R),:)+pinv(A(1:sum(R),1:sum(R)) - 0.001*eye(sum(R))      )*phi*Err;
